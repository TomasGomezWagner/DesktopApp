...
class VistaPrincipal(customtkinter.CTk):
    def __init__(self):
        ...
        self.txt_btn = customtkinter.CTkButton(
            self.frame_izquierda,
            text='Filtrar txt',
            command=self.frame_txt
            )
        
        self.txt_btn.grid(column=0, row=2, pady=10, padx=10)
        
        self.img_btn = customtkinter.CTkButton(
            self.frame_izquierda,
            text='Filtrar imagenes',
            command=self.frame_img,
            )
    ...
    def frame_txt(self,):
        ...
        self.btn_examinar = customtkinter.CTkButton(
            self.frame_derecha_txt,
            text='EXAMINAR',
            command=lambda:examinar(self.ruta_txt)
            )
        ...
        self.btn_procesar = customtkinter.CTkButton(
            self.frame_derecha_txt,
            text='PROCESAR',
            command=lambda:Principales.filtrar_archivo_txt(self.ruta_txt.get())
            )
    ...
    def frame_img(self,):
        ...
        self.btn_examinar = customtkinter.CTkButton(
            self.frame_derecha_img,
            text='EXAMINAR', 
            command=lambda:examinar(self.ruta_descarte)
            )
        ...
        self.btn_examinar_carpeta = customtkinter.CTkButton(
            self.frame_derecha_img,
            text='EXAMINAR',
            command=lambda:examinar_carpeta(self.ruta_img)
            )
    ...
    def frame_dppsv(self,):
        ...
        self.titulo_lbl = customtkinter.CTkLabel(
            self.frame_derecha_dppsv,
            text='DEVOLUCION DPPSV'
            )
        ...
        self.entry_txt = customtkinter.CTkEntry(
            self.frame_derecha_dppsv,
            textvariable=self.txt_dppsv
            )
        self.entry_txt.grid(column=0, row=2, sticky='we', padx=20)

        self.btn_txt = customtkinter.CTkButton(
            self.frame_derecha_dppsv,
            text='examinar',
            command=lambda:examinar(self.txt_dppsv)
            )
        self.btn_txt.grid(column=0, row=3, sticky='we', padx=70, pady=10)

        self.label_pdf = customtkinter.CTkLabel(
            self.frame_derecha_dppsv,
            text='Ruta de carpeta de PDF'
            )
        self.label_pdf.grid(column=0, row=4)

        self.entry_pdf = customtkinter.CTkEntry(
            self.frame_derecha_dppsv,
            textvariable=self.pdfs_dppsv
            )
        ...
        self.btn_pdf = customtkinter.CTkButton(
            self.frame_derecha_dppsv,
            text='examinar',
            command=lambda:examinar_carpeta(self.pdfs_dppsv)
            )
...
#-------------------------------------------------------------------------------------------------------------------
    ...
    def open_toplevel_progress(self,) -> None:
        
        if self.toplevel_progress is None or not self.toplevel_progress.winfo_exists():
            self.toplevel_progress = TopLevelProgressBar(self.txt_dppsv.get(),self.pdfs_dppsv.get())
            self.toplevel_progress = None
